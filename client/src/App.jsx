...
import { Camera, Upload as IconUpload, User, LogOut, LogIn, Search } from 'lucide-react'
      ...
      <div className="nav-links">
        <div className="nav-search">
          <input
            ...
            onKeyDown={(e)=>{ if (e.key === 'Enter') navigate(navQuery ? `/?q=${encodeURIComponent(navQuery)}` : '/') }}
            placeholder="搜索作品"
            aria-label="搜索作品"
            style={{ paddingLeft: 12 }}
          />
          <button className="btn btn-primary" onClick={()=>navigate(navQuery ? `/?q=${encodeURIComponent(navQuery)}` : '/')} style={{ padding: '8px 12px' }}>
            <Search size={16} />
          </button>
        </div>
        {token && (
