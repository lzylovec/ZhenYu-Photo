...
## 技术栈
- 后端：FastAPI、PostgreSQL (Supabase)、MinIO/R2（对象存储）、Uvicorn
- 前端：React、Vite、React Router、Axios

## 目录结构
- `python_server/` 后端代码
- `client/` 前端代码
- `uploads/` 本地回退存储（未配置 MinIO/R2 时使用）
...
- 后端（开发）
  ...
  - 配置环境变量（`.env.local`）：
    - `DATABASE_URL`: PostgreSQL 连接字符串 (例如: `postgresql://user:pass@host:port/db`)
  - 启动：
...
- MinIO/Cloudflare R2（可选，配置后优先使用对象存储）
  - 详见 `docs/minio-config.md` 或参考 `.env.local` 中的 R2 配置
...
## 常见问题
- 前端显示不出视频/图片：检查资源 URL 前缀是否为 MinIO/R2 公网地址或本地 `uploads/`；若 403，请确认桶策略允许 GetObject。
- 上传失败：确认登录身份为 `super_admin` 并携带 CSRF；视频上传仅允许 `video/*` MIME 类型。
- 连接数据库失败：确认 `DATABASE_URL` 配置正确，且网络能够访问数据库主机（特别是 Supabase Pooler 地址）。

## 安全建议
- 不要把密钥与口令写入仓库；使用进程环境变量或安全配置管理。
- 如需外网访问，建议为对象存储配置合适的策略与域名。

## 代码位置提示
- 对象存储客户端与配置：`python_server/main.py:51-79`
